...
import chromadb
import requests
# obj = requests.get(f'http://api.conceptnet.io/c/en/system').json()
# print(obj['edges'][7])
# exit()
    ...
    for area, qs in tqdm(areas.items()):
        ...
        for question, concepts in zip(qs["questions"], qs["concepts"]):
            ...
            for concept in concepts:
                ...
                # for page in pages[:3]:
                try:
                    # print(f"Loading obj {concept}")
                    obj = requests.get(f'http://api.conceptnet.io/c/en/{concept}').json()
                    text = "\n".join([i['surfaceText'] for i in obj['edges'] if i['surfaceText']])
                    conceptnet_docs.append(Document(id_=str(id), text=text))
                    id += 1
                except Exception as e:
                    print(f"Error loading obj {concept}: {e}")
                    # doc = WikipediaReader().load_data(pages=[concept])
                
                # conceptnet_docs.extend(results)
            # print(len(conceptnet_docs))
            # exit()
            # print(text)
            print(conceptnet_docs)
            exit()
            ...
            index = VectorStoreIndex.from_documents(conceptnet_docs)
...
with open("quiz_concept_conceptnet.json", "w") as f:
